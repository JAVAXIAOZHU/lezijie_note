...
import comlezijie.note.dao.NoteTypeDao;
import comlezijie.note.po.NoteType;
...
public class NoteTypeService {
    ...
    public List<NoteType> findTypeList(Integer userId){
       //1. 调用Dao层的查询方法，通过用户ID查询类型集合
        List<NoteType> typeList=typeDao.findTypeLsitByUserId(userId);
       //2. 返回类型集合
        return typeList;
    }
    ...
    public ResultInfo<NoteType> deleteType(String typeId) {
        ...
        if (noteCount>0){
            //code=0，msg="该类型存在子记录，不可删除"，返回resultInfo对象
            resultInfo.setCode(0);
            resultInfo.setMsg("该类存在子记录，不可删除！");
            return resultInfo;
        }
        //4. 如果不存在子记录，调用Dao层的更新方法，通过类型ID删除指定的类型记录，返回受影响的行数
