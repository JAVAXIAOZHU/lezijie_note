...
public class UserService {
    ...
    //数据回显：当登录实现时，将登陆信息返回给页面显示
        ...
        //2.如果不为空  通过用户名查询用户对象
        User user=userDao.queryUserByName02(userName);

        //3.判断用户对象是否为空
        if (user==null){
            //如果为空，设置ResultInfo对象的状态码和提示信息
            resultInfo.setCode(0);
            resultInfo.setMsg("该用户为空！");
            //返回resultInfo对象
            return resultInfo;
        }

        //4.如果对象不为空，将数据库中查询到的用户对象的密码与前台传递的密码作比较  （将密码加密后再比较）
